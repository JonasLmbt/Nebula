...
class HypixelApiRouter {
  ...
  private async fetchFromBackend(name: string): Promise<any> {
    if (!this.config.backendUrl) throw new Error('No backend URL configured');
    ...
    try {
      ...
      const response = await fetch(`${this.config.backendUrl}/api/player/${encodeURIComponent(name)}`, {
        method: 'GET',
